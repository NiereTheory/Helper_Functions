...
date_int_format: str = r"%Y%m%d"
date_str_format: str = r"%Y-%m-%d"


def fns_date(date_id: int) -> str:
...
def fns_date_id(dt: date):
    """Convert a python date to an integer date_id"""
    return int(dt.strftime(date_int_format))


def gen_dim_date(start_date: date = date(2019, 1, 1), end_date: date = date(2019, 12, 31)) -> List[Dict]:
    ...
    for i in range(delta.days + 1):
        ...
        python_date: date = start_date + timedelta(days=i)
        ...
        current_date_dict["IS_WEEKDAY"] = 1 if python_date.weekday(
        ) <= 4 else 0
        ...
        current_date_dict["FIRST_DAY_OF_QUARTER"] = python_date.replace(
        ...
        final_date_dict: Dict = {
            k: fns_date_id(v) if k.startswith(("FIRST", "LAST")) else v
    ...
    try:
        destination_csv_path: str = os.path.join(
            *[
                "/Users",
                getpass.getuser(),
                "Projects/Helper_Functions/dim_date/dim_date.csv",
            ]
        )
